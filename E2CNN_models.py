...
class RegBlockRN18(nn.EquivariantModule):
    '''
    ...
    def __init__(self, 
        ...
        self.c1 = nn.R2Conv(in_type = in_type, out_type = out_type, kernel_size = kernel_size, stride = stride, padding = padding, bias = False)
        self.n1 = nn.InnerBatchNorm(out_type)
        self.a1 = nn.ReLU(out_type)

        self.c2 = nn.R2Conv(in_type = out_type, out_type = out_type, kernel_size = kernel_size, stride = stride, padding = padding, bias = False)
        self.n2 = nn.InnerBatchNorm(out_type)
        self.a2 = nn.ReLU(out_type)

        if stride != 1 or in_type != out_type:
            self.skipcon = nn.R2Conv(in_type = in_type, out_type = out_type, kernel_size = 1, stride = stride, padding = 0, bias = False)
...
class RegRepRN18(tnn.Module):
    ...
    def __init__(self, n0=64, n1=9, kernel_size=3, img_channels=3):
        ...
        in_type = nn.FieldType(self.r2_act, img_channels * [self.r2_act.trivial_repr])
        out_type = nn.FieldType(self.r2_act, n0 * [self.r2_act.regular_repr])
        self.in_type = in_type
        ...
        self.inblock1 = nn.SequentialModule(
            nn.R2Conv(in_type = in_type, out_type = out_type, kernel_size = kernel_size, stride = 1, padding = padding, bias = False),
    ...
    def _make_layer(self, block, planes, blocks, totrivial = False) -> nn.SequentialModule:
        '''
        ...
        inner_type = nn.FieldType(self.r2_act, planes * [self.r2_act.regular_repr])

        # Specify the out_type according to the representation
        if totrivial:
            out_type = nn.FieldType(self.r2_act, planes * [self.r2_act.trivial_repr])
        else:
            out_type = nn.FieldType(self.r2_act, planes * [self.r2_act.regular_repr])
