...
class BlockRN18(enn.EquivariantModule):
    '''
...
class RegRepRN18(tnn.Module):
    ...
    def __init__(self, n0=64, n1=9, kernel_size=3, img_channels=3):
        ...
        self.layer2 = self._make_layer(BlockRN18, self.stages[0], self.n[0], kernel_size = kernel_size, padding = padding)

        self._in_type = self.layer2.out_type
        self.layer3 = self._make_layer(BlockRN18, self.stages[1], self.n[1], kernel_size = kernel_size, padding = padding)

        self._in_type = self.layer3.out_type
        self.layer4 = self._make_layer(BlockRN18, self.stages[2], self.n[2], kernel_size = kernel_size, padding = padding)

        # The last layer has the trivial representation as the output
        self._in_type = self.layer4.out_type
        self.layer5 = self._make_layer(BlockRN18, self.stages[3], self.n[3], kernel_size = kernel_size, padding = padding, totrivial = True)
